...
> RESPONSE: The years covered in the spending dataset are 1991-2014, and the years covered in the coverage dataset are 2013-2016. 

```{r}
relevantSpending <- spending %>% select(Location, starts_with("2013"), starts_with("2014"))
relevantSpending
relevantCoverage <- coverage %>% select(Location, starts_with("2013"), starts_with("2014"))
relevantCoverage
...
```{r}
relevantSpending <- relevantSpending %>% 
...
> RESPONSE:  The scatterplot will require each row to be a state, with columns being year, Health spending, and Employer coverage.


What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: The data will need to be narrowed and then joined, as seen below.
...
```{r}
coverageNarrow <- relevantCoverage %>% 
  select(Location, "2013" = "2013__Employer", "2014" = "2014__Employer")  %>%
  gather(key = "Year", value = "Employer Coverage", "2013", "2014")

coverageNarrow

spendingNarrow <- relevantSpending %>% 
  rename("2013" = "2013__Total Health Spending", "2014" = "2014__Total Health Spending") %>% 
  gather(key = "Year", value = "Health Spending", "2013", "2014")

spendingNarrow

spendingAndCoverage <- coverageNarrow %>% inner_join(spendingNarrow, by = c("Location" = "Location", "Year" = "Year"))

spendingAndCoverage <- spendingAndCoverage %>% clean_names()



ggplot(data = spendingAndCoverage, mapping = aes(x = log(employer_coverage), y = log(health_spending))) + geom_point(color = "dark blue")
