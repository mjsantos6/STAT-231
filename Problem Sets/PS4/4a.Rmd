...
> ANSWER: There appear to be 12 tables.


```{r}
#disaster
url = read_html("https://en.wikipedia.org/wiki/Mile_run_world_record_progression")
tables = url %>% html_nodes(css="table")
length(tables)
```
...
> ANSWER: The indoor one-mile world record for IAAF women is held by Genezebe Dibaba and her time was 4:13.31
...
> ANSWER: The fastest time was 4:12.33 by Sifan Hassan, thus it was from an outdoor event.

```{r}
women_outdoor = html_table(tables[[8]])
women_outdoor$type = ("Outdoor")
women = bind_rows(women_outdoor, women_indoor)
min(women$Time)
```