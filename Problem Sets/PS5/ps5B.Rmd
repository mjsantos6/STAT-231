...
author: "Mike Santos"
date: "due by 2 PM on Friday, October 2"
output:
...
length(tables)
Justices <- html_table(tables[[0]], fill = TRUE)
write_csv(Justices, "justices.csv", append = FALSE)
...
> ANSWER: This plot shows that most U.S. Supreme Court justices serve 

```{r}
justices <- read_csv("~/Desktop/STAT-231/PS5/justices.csv")

justices2 <- justices %>%
  clean_names() %>%
  # remove extra line that comes in at second column of table
...
library(ggplot2)
ggplot(justices2, aes(x=tenure_length)) + geom_histogram(binwidth=1, color = "red", fill = "navy")
summary(justices2$tenure_length)
...
```{r, linewidth = 60}
quotes_dat <- read_csv("~/Desktop/STAT-231/PS5/quotes.csv")
